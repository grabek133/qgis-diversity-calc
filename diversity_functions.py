...
def dc_mergeDictionaries(dMain, cat, dPoly):
    ...
    if cat in dMain.keys():
        # if it does loop through the summary data in dPoly
        for species, obs in dPoly.items():
            ...
            if species in dMain[cat].keys():
                # if there is then add the number observation in the summary data
                dMain[cat][species] += obs
            else:
                # if there isn't the create a new entry for tge species and set the
                # number of observation as the initial value
                dMain[cat][species] = obs
    else:
